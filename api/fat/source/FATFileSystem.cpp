...
#include "../include/fat/FATFileSystem.h"
...
static char * fat_path_prefix(int id, const char *path)
{
...
static arduino::BlockDevice *_ffs[FF_VOLUMES] = {0};
...
extern "C" DRESULT disk_read(BYTE pdrv, BYTE *buff, DWORD sector, UINT count)
{
    ...
    arduino::bd_addr_t addr = (arduino::bd_addr_t)sector * ssize;
    arduino::bd_size_t size = (arduino::bd_size_t)count * ssize;
    ...
    switch (cmd) {
        ...
        case CTRL_TRIM:
            if (_ffs[pdrv] == NULL) {
                ...
                DWORD ssize = disk_get_sector_size(pdrv);
                arduino::bd_addr_t addr = (arduino::bd_addr_t)sectors[0] * ssize;
                arduino::bd_size_t size = (arduino::bd_size_t)(sectors[1] - sectors[0] + 1) * ssize;
...
{
    char * fpath = fat_path_prefix(_id, path);
...
{
    char * oldfpath = fat_path_prefix(_id, oldpath);
    char * newfpath = fat_path_prefix(_id, newpath);
